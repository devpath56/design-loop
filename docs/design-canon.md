...
**Sibling spine:** [`competency-map.md`](./competency-map.md) is the IC-principal **design-engineer**
competency set with the most precise VERIFIED source per competency (chapters/essays/specs/cases, not
whole books) — the front-end-craft half this canon does not carry (modern CSS, tokens, Core Web Vitals,
inclusive components). Ground a competency-specific claim in the map; ground a teaching-source choice here.
...
| 4 | **Wathan & Schoger, *Refactoring UI* (2018)** for a developer audience — or **Robin Williams, *The Non-Designer's Design Book* (Peachpit, 4th ed. 2015)** (the four principles — contrast, repetition, alignment, proximity — as standalone chapters; "CRAP" is a common third-party mnemonic, **not** Williams' own term) for a general one | visual / UI craft — hierarchy, spacing, colour, type | The visual-craft axis the academic canon ignores. **Not a foundational-vs-companion ranking** (an earlier draft's error, corrected). Both cover the *same* visual-craft layer, indexed differently: Williams teaches the four print/graphic fundamentals as transferable *theory* for any reader; *Refactoring UI* hands a **developer** copy-pasteable UI recipes and is explicitly *not* a fundamentals deep-dive. Pick by **audience × need**. For this loop — a dev/PM learning design — **Refactoring UI is the working primary** (tactics in the learner's own language); Williams is the *why-it-works* backstop that transfers off the web. | `[VERIFIED — Refactoring UI: https://www.refactoringui.com/ (dev-targeted tactics, per publisher + reviews) · Williams: https://www.peachpit.com/store/non-designers-design-book-9780133966152, accessed 2026-07-20]` |
...
| Robin Williams, The Non-Designer's Design Book, Peachpit 4th ed. 2015 (four principles: contrast, repetition, alignment, proximity — "CRAP" is a third-party mnemonic, not her term) | VERIFIED (this session; added as the general-audience counterpart to Refactoring UI's dev-targeted tactics — same layer, different reader, NOT ranked above it) |
